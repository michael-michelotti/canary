 ...
 *
 * Driver for the Texas Instruments TMP116/TMP117/TMP119 Temperature Sensors
 * (7-bit I2C slave address (0x48 - 0x4B), changeable via ADD pins)
 *
 * Note: This driver assumes that the sensor has been calibrated beforehand.
 *
 * TMP119 support added by Michael Michelotti, 2026. TMP117 and TMP119 are
 * register-identical; the device ID register (0x0F) packs a 12-bit device ID
 * in the low bits and a 4-bit silicon revision in the high bits. TMP117
 * reports revision 0 (ID = 0x0117), TMP119 reports revision 2 (ID = 0x2117).
 * The original driver compared the whole word and fell through to the
 * "unknown ID" fallback for TMP119; this version masks off the revision
 * bits so both parts match explicitly.
 */
 ...
 *   [15:12] silicon revision  (0 = TMP117, 2 = TMP119)
 *   [11: 0] device ID
 */
#define TMP11X_DEVICE_ID_MASK		0x0FFF
#define TMP11X_REV_MASK			0xF000
#define TMP11X_REV_SHIFT		12

#define TMP116_DEVICE_ID		0x116
#define TMP117_DEVICE_ID		0x117
#define TMP119_REVISION			0x2
...
/* TMP119 shares the TMP117 register map and feature set; only the name differs. */
...
static int tmp117_probe(struct i2c_client *client)
{
	...
	dev_id_raw = i2c_smbus_read_word_swapped(client, TMP117_REG_DEVICE_ID);
	if (dev_id_raw < 0)
		return dev_id_raw;

	dev_id = dev_id_raw & TMP11X_DEVICE_ID_MASK;
	rev = (dev_id_raw & TMP11X_REV_MASK) >> TMP11X_REV_SHIFT;
	...
	case TMP117_DEVICE_ID:
		/* Same base ID; revision distinguishes TMP117 (0) from TMP119 (2). */
		if (rev == TMP119_REVISION)
...
static const struct of_device_id tmp119_of_match[] = {
...
MODULE_DEVICE_TABLE(of, tmp119_of_match);

static const struct i2c_device_id tmp119_id[] = {
...
MODULE_DEVICE_TABLE(i2c, tmp119_id);

static struct i2c_driver tmp119_driver = {
	.driver = {
		.name	= "tmp119",
		.of_match_table = tmp119_of_match,
	},
	.probe		= tmp117_probe,
	.id_table	= tmp119_id,
};
module_i2c_driver(tmp119_driver);

MODULE_AUTHOR("Puranjay Mohan <puranjay12@gmail.com>");
MODULE_DESCRIPTION("TI TMP116/TMP117/TMP119 Temperature sensor driver");
MODULE_LICENSE("GPL");
